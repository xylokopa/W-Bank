# 04_twin_plotter_02c.py
# =====================================================================
# MODUL 4 (TEIL 1 VON 2): 9-BUTTON INTERFACE & COMPUTATION
# =====================================================================
...
# Dynamische Erkennung aller 9 Variablen aus dem Block-Format
...
################################################################################
# =====================================================================
# MODUL 4 (TEIL 2a): GRAPHICAL ENGINE & PRECISE SCALAR ARROWS
# =====================================================================
...
def draw_eigen_vectors(ax, evecs, evals, is_dataset_b=False):
    """Extrahiert die 3D-Komponenten als nackte Skalare für ax.arrow"""
    ...
    # DIE RETTUNG: Expliziter Index-Abgriff speist die Funktion mit echten Skalaren!
...
def update_plots(full_refresh=True):
    ...
    if full_refresh:
        ...
        if is_3d_mode and len(active_feats) == 3:
            draw_background_axes(ax_left, active_feats)
        ...
        ax_left.set_title(f"A: MASTER WOLKE")

        for i in range(len(x_plot_A)):
            ax_left.text(
                x_plot_A[i] + 0.05,
                y_plot_A[i] + 0.05,
                str(df_orig["iso3"].iloc[i]),
                fontsize=7,
                alpha=0.3,
            )

        scatter_B = ax_right.scatter(
            x_plot_B, y_plot_B, alpha=0.6, color="royalblue"
        )
        (highlight_redu,) = ax_right.plot(
            [], [], "ro", markersize=10, zorder=12
        )
        ax_right.set_title(f"B: REDUZIERT (QUAD-PRUNER)")

        for i in range(len(x_plot_B)):
            ax_right.text(
                x_plot_B[i] + 0.05,
                y_plot_B[i] + 0.05,
                str(df_redu["iso3"].iloc[i]),
                fontsize=8,
                alpha=0.5,
            )
        ...
        if is_3d_mode:
            draw_eigen_vectors(
                ax_left, evecs_A_raw, evals_A_raw, is_dataset_b=False
            )
            draw_eigen_vectors(
                ax_right, evecs_B_raw, evals_B_raw, is_dataset_b=True
            )
        ...
        for ax, cov, name in [
            ...
            ax.set_title(f"Kov-Matrix {name}", fontsize=10)
            ax.set_xticks(range(len(active_feats)))
            ax.set_yticks(range(len(active_feats)))
            ax.set_xticklabels(
                active_feats, rotation=25, ha="right", fontsize=8
            )
        ...
        if scatter_B is not None:
            ...
            if is_3d_mode:
                for p in list(ax_right.patches):
                    p.remove()
                draw_background_axes(ax_right, active_feats)
                draw_eigen_vectors(
                    ax_right, evecs_B_raw, evals_B_raw, is_dataset_b=True
...
################################################################################
    # =====================================================================
# MODUL 4 (TEIL 2b): INTERACTIVE COCKPIT WIDGETS & CALL-BACKS
# =====================================================================
...
for idx, feat in enumerate(ALL_FEATURES):
    ax_btn = plt.axes([x_start + (idx * 0.046), 0.03, 0.042, 0.04])
    btn = Button(ax_btn, feat[:6], color="orchid")
    silence_hover(btn)
...
def make_toggle_callback(f_name):
    def callback(event):
        ...
        if filter_lock:
            return
        if sum(feature_states.values()) <= 2 and feature_states[f_name]:
            return
        filter_lock = True
...
ax_btn_restart = plt.axes([0.43, 0.03, 0.05, 0.04])
...
def on_restart(event):
    ...
    if filter_lock:
        return
    filter_lock = True
    rotation_state, mirror_state = 0, 1
    idx_focus_A, idx_focus_B = None, None
    for f in ALL_FEATURES:
...
ax_box_l = plt.axes([0.50, 0.03, 0.04, 0.04])
text_box_l = TextBox(ax_box_l, "ISO A: ", initial="")
ax_box_r = plt.axes([0.59, 0.03, 0.04, 0.04])
text_box_r = TextBox(ax_box_r, "ISO B: ", initial="")

#############################################################################
# =====================================================================
# MODUL 4 (TEIL 2b): INTERACTIVE WIDGET COCKPIT (PROT_COMPACT)
# =====================================================================
def submit_left(text):
    global idx_focus_A
    try:
        val = str(text).strip().upper()
        match = df_orig[df_orig["iso3"] == val]
...
def submit_right(text):
    ...
    try:
        val = str(text).strip().upper()
        match = df_redu[df_redu["iso3"] == val]
...
# GEOMETRIE KNOEPFE GANZ RECHTS PLATZIEREN
ax_btn_rot = plt.axes([0.68, 0.03, 0.11, 0.04])
btn_rotate = Button(ax_btn_rot, "Drehen (90°)", color="dodgerblue")
silence_hover(btn_rotate)

ax_btn_mir = plt.axes([0.81, 0.03, 0.15, 0.04])
...
# INITIALER ZUENDSCHLUESSEL (ALLES BEREIT)
update_plots(full_refresh=True)
plt.show()
